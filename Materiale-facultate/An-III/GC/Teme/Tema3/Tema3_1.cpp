...
void dreptunghi(void)
{
	...
	glTranslated(i, 200.0 + 0.5 * i, 0.0); // generata o matrice 4x4 T, corespunzatoare translatiei, inm. cu id
...
void dreptunghi2(void)
{
	...
	// deplasare pe diagonala
	glTranslated(i, 200.0 + 0.5 * i, 0.0); // generata o matrice 4x4 T, corespunzatoare translatiei
...
void miscas(void)
{
	...
	if (i < 0.0)
		step = alpha;
	else if (i > 750.0) // marim limita superioara
		step = -alpha;
	j = j + step;
	glutPostRedisplay();
}
...
void main(int argc, char** argv)
{
	...
	glutCreateWindow("Tema 3 - Dreptunghi cu satelit");
	init();
